...
1. 有技术创新或突破的方案：防篡改Sign验证、RSA加密保护敏感数据、IP限流
	...
	- 关键实现细节：
		- 后端生成RSA公私钥对，私钥存Session，公钥经AES加密返回前端；每次接口调用后动态更新私钥有效期，避免长期存储风险。
		- 设计了多个Java拦截器和切面类，比如LoginIpInterceptor负责IP限流，DeviceFingerprintInterceptor负责设备指纹校验，ParamSignCheckAspect负责参数签名验证
	- 最终实现效果：成功保障工资单系统的稳定和安全
